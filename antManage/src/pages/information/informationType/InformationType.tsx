      ...
      return (
        ...
        >
          <Form layout="vertical">
            <Form.Item label="信息分类名称">
              {getFieldDecorator('name', {
                initialValue: dataInfo ? dataInfo.name : '',
                rules: [{ required: true, message: '请输入信息分类名称！' }],
              })(<Input />)}
            </Form.Item>
            {dataInfo ? null : (
              <Form.Item style={{ marginBottom: 15 }} label="上级分类(一级分类时不用选择)">
                {getFieldDecorator(
                  'parent_id',
                  {},
                )(
...
class InformationType extends React.Component<FormComponentProps> {
  columns = [
    {
      title: '信息分类id',
      dataIndex: 'id',
  ...
  createData = (values: any) => {
    if (!this.loginData) return;
    return fetch('/api/information/category/create', {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: this.token,
  ...
  editData = (values: any) => {
    if (!this.loginData) return;
    return fetch('/api/information/category/update', {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: this.token,
...
export default Form.create<FormComponentProps>()(InformationType);
