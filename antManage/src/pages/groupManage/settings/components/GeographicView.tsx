...
@connect(
  ({
    ...
    const { province, city } = groupManageAndsettings;
    return {
      province,
      city,
      loading: loading.models.groupManageAndsettings,
    };
  },
)
class GeographicView extends Component<GeographicViewProps> {
  componentDidMount = () => {
    ...
    if (dispatch) {
      dispatch({
        type: 'groupManageAndsettings/fetchProvince',
      });
    }
  };
  ...
  componentDidUpdate(props: GeographicViewProps) {
    ...
    if (!props.value && !!value && !!value.province) {
      if (dispatch) {
        dispatch({
          type: 'groupManageAndsettings/fetchCity',
