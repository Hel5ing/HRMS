...
class UserInfoManage extends React.Component<FormComponentProps> {
  columns = [
    {
      title: '员工工号',
    ...
    },
    {
      ...
      dataIndex: 'site',
      key: 'site',
      render: (data: SiteInfo) => <div>{data ? data.name : ''}</div>,
    },
    {
      title: '邮箱',
      dataIndex: 'email',
  ...
  render() {
    if (!this.state.personInfoList || !this.state.groupTabList?.length) {
      return <PageLoading />;
    }
...
export default Form.create<FormComponentProps>()(UserInfoManage);
